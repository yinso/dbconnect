      ...
      try {
        ...
        return this.query(query, res, function(err, results) {
          if (err) {
            ...
            console.log("" + _this.constructor.name + ".insert:result", results instanceof Array);
            if (results instanceof Array) {
              return cb(null, _this.schema.makeRecordSet(_this, tableName, results));
            } else {
        ...
        return this.query(query, {}, function(err, results) {
          try {
            if (err) {
              ...
              console.log("" + _this.constructor.name + ".select", tableName);
              return cb(null, _this.schema.makeRecordSet(_this, tableName, results));
            }
          } catch (err) {
            return cb(err);
